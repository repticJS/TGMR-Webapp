...
export function buildAchievementCatalog(completedAchievements = []) {
  ...
  for (const achievement of ACHIEVEMENT_CATALOG) {
    index.set(`${achievement.world_id || "default"}::${achievement.achievement_id}`, achievement);
  }

  for (const completed of completedAchievements) {
    const worldId = completed.world_id || "default";
    const key = `${worldId}::${completed.achievement_id}`;
